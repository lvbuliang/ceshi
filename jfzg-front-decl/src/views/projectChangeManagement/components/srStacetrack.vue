    ...
    <div class="sr-trace-track-dialog">
      ...
      <div class="dialog-content">
        <template v-if="activeName == 'first'">
          ...
          <div class="emptyBox" v-else-if="trackData.length == 0">
            <img src="../../../assets/empty.png" alt />
            <div v-if="trackIsLoading" class="name_loader">
              <div>数据加载中，请稍后！</div>
            </div>
            <div v-else>暂无数据</div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      fileTypes: {
        view: ["doc", "docx", "txt", "html", "pdf", "ppt", "pptx", "ofd"],
        download_view: [
          "bmp",
          "jpg",
          "jpeg",
  ...
  created() {
    ...
    this.activeCurrent =
      statusMapping[this.$route.query.status] !== undefined
        ? statusMapping[this.$route.query.status]
        : this.activeCurrent;
  },
  methods: {
    download(file) {
    ...
    seeclick(file) {
      // 错误处理逻辑
      if (!file || !file.id || !file.name || !sessionStorage.getItem("token")) {
      ...
      const url = `${this.baseUrl}/jfzg/file/api/file/download?${params}`;

      if (this.fileTypes.view.includes(ext)) {
        window.open(`${this.baseUrl}/viewPdf?id=${file.id}`);
      } else if (this.fileTypes.download_view.includes(ext)) {
        window.open(url);
      } else {
        // 处理未知文件类型或显示错误消息
