  ...
  <div v-if="showWorkFlow" class="workflow flow">
    <div v-if="buttonList[0].showForm" class="workflow_formAdd">
      ...
      <div
        ...
        v-if="[2, 202, 203, 204, 301, 302, 303].includes(currentStatus)"
      >
        <SrAdd
      ...
      <div
        class="workflow_formAdd_left"
        v-if="[401, 402, 403, 1000].includes(currentStatus)"
      >
        <SrAdd
          v-if="buttonList[0].showForm"
        ...
        >
          <template slot="kkrq">
            <div style="width: 33%">
              <el-date-picker
                v-model="lastStepForm.data.workDate"
                type="date"
                format="yyyy-MM-dd"
...
import { api_queryOpinion, api_getDetail } from "@/api/projectStart/index.js";
...
export default {
  ...
  computed: {
    busiId() {
      return this.$route ? this.$route.query.projectStartId : "";
    },
    busiType() {
  ...
  mounted() {
    ...
    console.log(this.$route.query.status != null, "$route.query.status");
    ...
    if (this.$route.query.status == 2 || this.$route.query.status == 204) {
    ...
    } else if (this.$route.query.status == 301 || [401, '401'].includes(this.$route.query.status)) {
  ...
  created() {
    ...
    if (this.buttonList[0].showForm) {
      ...
      this.getTraceTrack();
      console.log(this.trackData, "trackData");
  ...
  methods: {
    ...
    getDetail() {
      if (![401, "401", 1000, "1000"].includes(this.$route.query.status))
        return false;
      ...
      api_getDetail(params).then((res) => {
        if (res.data) {
          //回显数据
          console.log("回显数据", res.data);
          const { workDecide, otherFileThree } = res.data;
          this.lastStepForm.data.workDate = res.data.workDate;
          if (workDecide == null) {
            this.lastStepForm.data.workDecideList = [];
          } else {
            this.lastStepForm.data.workDecideList = setFile(workDecide);
          }
          if (otherFileThree === null) {
            this.lastStepForm.data.otherFileList = [];
          } else {
            this.lastStepForm.data.otherFileList = setFile(otherFileThree);
          }
        }
      });
    },
    ...
    getAuditOpinion() {
      let params = {
        projectStartId: this.busiId,
        // processInstanceId: this.processId,
        // taskId: this.processId,
      };
      api_queryOpinion(params).then((res) => {
        if (res.data) {
          this.form.data.auditOpinion = res.data.opinionInfo;
          this.lastStepForm.data.auditOpinion = res.data.opinionInfo;
          console.log('走到这一步了吗', res.data);
    ...
    getParam() {
      ...
      console.log(item);
      let param = {
        vars: item.btnValue, //"{'sh_flag':0}"
        processInstanceId: this.processId, //processId
        taskId: this.taskId, //taskId
        // status: this.currentStatus,
        btnName: item.btnName, //操作按钮的详情
        auditOpinion: [401, '401'].includes(this.$route.query.status) ? this.lastStepForm.data.auditOpinion : this.form.data.auditOpinion, //获取审核意见
      };
      if (item.nextBuCode) {
      ...
      if (this.clickMethods && this.clickMethods[item.btnEventType]) {
        ...
        if (this.buttonList[0].showForm) {
          if (![401, 402, 403, 1000].includes(this.currentStatus)) {
            ...
            this.lastStepForm.data.workDecideList.map((item) => {
            ...
            param = {
              ...
              workDate: this.lastStepForm.data.workDate,
              workDecide,
              otherFileThree,
      ...
      if (!this.busiType || !this.busiId) {
        ...
        this.srtracetrackRequest.getTraceTrackList(
          ...
          (res) => {
            ...
            console.log(this.trackData, "trackDatatrackData");
            this.trackLoading = false;
          }
        );
      }
