...
export default {
  ...
  methods: {
    ...
    changechecbox2(e, item, buName, buid, id, index, index1) {
      //console.log(e, item, buName, buid, id, index, index1, this.selectdata);
      this.$set(item, "isShow", item.isShow);
      if (e == true) {
        const index4 = this.selectdata.findIndex(
          (item4) => item4.buid === buid
        );
        //console.log("a>>".index4);
        if (index4 !== -1) {
          //console.log("index4", index4);
          this.selectdata[index4].data.push(item);
        } else {
          //console.log("2.......");
          let obj = {
            buName,
        ...
        this.selectdata.forEach((element, sub) => {
          const index3 = element.data.findIndex(
            (ele) => ele.jfid === item.jfid
          );
          //console.log("index3", index3);
          if (index3 !== -1) {
            //console.log("element", element, sub);
            if (element.data.length == 1) {
              this.selectdata.splice(sub, 1);
            } else {
              element.data.splice(index3, 1); // 从 expertList 中移除对应的项
            }
          }
        });
      }
      //console.log("选择数据", this.selectdata);
    },
