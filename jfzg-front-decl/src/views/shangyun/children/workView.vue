<template>
  <div class="total-page workview">
    <srNavPage :type="2" :project="srNavPageObj"></srNavPage>
    <tabFormView
      class="tabFormView"
    ...
    <WorkFlow
      ...
      :backName="$route.name.includes('Audit') ? 'shangyunAuditList' : 'shangyunList'"
    ></WorkFlow>
  </div>
</template>
...
import { queryShangyunDetail } from "@/api/shangyun/index";
export default {
  components: {
    tabFormView,
    WorkFlow,
  ...
  created() {
    queryShangyunDetail({ id: this.$route.query.id }).then((res) => {
