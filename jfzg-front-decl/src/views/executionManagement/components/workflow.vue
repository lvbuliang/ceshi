<template>
  <div v-if="showWorkFlow" class="workflow flow">
    <div class="workflow_formAdd">
      <!-- v-if="(buttonList[0].showForm && $route.query.flog != 1) ||
          $route.query.flag == 0" -->
      <div v-if="true
        " class="workflow_formAdd_left">
        <!-- v-if="buttonList[0].showForm || $route.query.flag == 0" -->
        <SrAdd v-if="true" :config="form.config" :otherConfig="{ labelNum: 80 }" :form="form.data" :enums="form.enums"
          ref="add" class="add">
          <div slot="inputBox">
            <!-- <el-popover
                placement="top"
            ...
            <el-input class="inputBox" v-model="form.data.auditOpinion" slot="reference" placeholder="请输入意见"
              maxlength="500" type="textarea" :autosize="{ minRows: 5, maxRows: 7 }" show-word-limit @focus="inputclick2">
            </el-input>
            <!-- </el-popover> -->
          </div>
          <!-- <div slot="inputBox2">
      ...
      <!-- <el-popover
        ...
        >
          ...
          <div slot="reference"  class="markBtton">
    ...
    <div class="workflow_buttons">
      <div class="automaticSave" v-if="typeof loading === 'boolean'">
        <div v-if="loading">
          <i class="el-icon-circle-check" style="margin-left: 4px; margin-right: 4px; color: #000000"></i>
          {{ getToTime }}自动保存成功
        </div>
        <div v-else><i class="el-icon-loading"></i>自动保存中</div>
      </div>
      <div class="workflow_button btn" :key="index" v-for="(item, index) in buttonList[0].btns">
        <!-- <el-dialog
        ...
        <el-button v-if="item.btnEventType == 'fp'" v-popover:popover5 @click.stop="goback(item)" :type="item.btnType">{{
          item.btnName }}</el-button>
        <slot :name="item.slotName" :btn="item" v-else-if="item.slotName">
        </slot>
        <!-- 通过不通过 -->
        <el-button :disabled="item.disabled" v-else @click.stop="goback(item)" :type="item.btnType">{{ item.btnName
        }}</el-button>
      </div>
    ...
    <!-- 审批留痕 -->
    <srStacetrack ref="track" class="track" :imgUrl="imgUrl" :trackData="trackData" :trackIsLoading="trackIsLoading"></srStacetrack>
...
// import srTrace from './srTrace.vue';
// import {
...
export default {
  ...
  //   // 根据状态动态显示审批意见和附件
  //   ///开始
  //   console.log("当前状态", this.currentStatus, this.$route);
  //   if (
  ...
  // },
  // created() {
  //   console.log("接收到的专家列表", this.expertViewList, this.$route);
  //   // 初始化单选按钮数据
  //   let userInfo = JSON.parse(sessionStorage.getItem("userInfo"));
  ...
  methods: {
    ...
    gobackClick(item) {
      ...
      if (this.clickMethods && this.clickMethods[item.btnEventType]) {
        ...
        console.log("替换后的param", param, item.btnEventType, this.validate, JSON.parse(this.userInfo).currentBizunit.buName);
        // debugger;
        ...
        param.projectId = this.$route.query.rowData.id; //项目id
...
.bzhTableHeader .el-dialog__body .content_box .content_left .content_left_lists .content_left_list {
  display: flex;
  width: 100%;
