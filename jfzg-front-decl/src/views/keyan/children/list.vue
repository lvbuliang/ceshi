...
export default {
  ...
  data() {
    return {
      ...
      codeObject: {
        //上传文件权限code
        updateFileCode: "M003_M004_005",
        //删除code
        deleteCode: "M003_M004_006",
        //停止上传code
        stopUploadingCode: "M003_M004_007",
      },
      //停止上传状态
        ...
        config: [
          {
            ...
            bind: "keywords", // 对应this.search.data对象的field1
          },
          {
            type: "select",
            placeholder: "请选择设计单位",
  ...
  created() {
    if (this.$route.name == "keyanAudit") {
      // this.search.btns = [
      //   { name: "导出", click: this.export, permissionCode: "M004_M004_001" },
      // ];
      this.search.config = [
        {
          type: "text",
      ...
      this.codeObject = {
        //上传文件权限code
        updateFileCode: "M004_M004_005",
        //删除code
        deleteCode: "M004_M004_006",
        //停止上传code
        stopUploadingCode: "M004_M004_007",
      };
  ...
  methods: {
    ...
    btnValidate4(row, item) {
      if (
        this.$utils.checkCodeInArr("M003_M004_004") ||
        this.$utils.checkCodeInArr("M003_M004_005") ||
        this.$utils.checkCodeInArr("M004_M004_004") ||
        this.$utils.checkCodeInArr("M004_M004_005")
      ) {
        return true;
      }
    },
    getTbData() {
      ...
      api_ky_queryList(param).then((res) => {
    ...
    export() {
      window.open(
        `${
          config.baseUrl
        }/jfzg/projectDecl/projectStudy/export?permissionCode=${permissionCode}&access_token=${sessionStorage.getItem(
          "token"
        )}`
      );
    },
    ...
    see(val) {
      this.$router.push({
        name: `${
          this.$route.name.includes("Audit") ? "keyanAuditView" : "keyanView"
        }`,
        query: {
          id: val.id,
          projectId: val.projectId,
