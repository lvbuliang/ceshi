...
import { api_queryOpinion } from "@/api/projectDisclosure/index.js";
export default {
  ...
  computed: {
    busiId() {
      return this.$route ? this.$route.query.discloseId : "";
    },
    busiType() {
  ...
  mounted() {
    ...
    if (this.$route.query.status == 2) {
      ...
      this.$set(this.form.config[0], "name", "确认人");
      this.$set(this.form.config[1], "name", "确认时间");
      this.$set(this.form.config[2], "name", "确认意见");
      // this.$set(this.form.config[2], "required", "确定意见必填！");
      this.$set(this.form.config[2], "placeholder", "请输入确认意见");
      // this.form.config.splice(-2, 2);
      // 终验附件
      this.form.config.splice(-3, 3);
  ...
  methods: {
      ...
      api_queryOpinion(params).then((res) => {
        ...
        if (res.data) {
          ...
          this.form.data.accessoryList = res.data.accessoryList
            ...
            : [];
          // 终验附件
          //   console.log(res.data.otherFiles,'res.data.otherFiles')
          this.form.data.otherListTwo = res.data.otherFiles
            ? res.data.otherFiles
            : [];
          // this.form.data.accessoryList.
          // const dicsData = this.tackdata.forEach((item) => {
          //   return res.data.discloseDate.push(item);
          // });
          this.form.data.discloseDate = res.data.discloseDate;
        }
      });
    },
    inputclick(el) {
    ...
    getParam() {
      ...
      let param = {
        ...
        // 终验附件
        otherFiles: this.form.data.otherListTwo,
      };
      if (item.nextBuCode) {
    ...
    fpSumbit() {
      ...
      if (this.buttonList[0].showForm) {
        this.form.data.accessory = this.form.data.accessoryList
          ...
          : [];
        // 终验附件
        this.form.data.otherFiles = this.form.data.otherListTwo
          ? this.form.data.otherListTwo.map((item) => {
              return item.id;
            })
          : [];
      ...
      if (this.clickMethods && this.clickMethods[item.btnEventType]) {
        ...
        if (this.form.data.accessoryList.length > 0) {
          this.form.data.accessoryList.forEach((item) => {
            item.deliveryName = "交底清单签字确认物";
          });
        }
        // 终验附件
        if (this.form.data.otherListTwo.length > 0) {
          this.form.data.otherListTwo.forEach((item) => {
        ...
        if (item.btnEventType != "bh" && item.btnEventType != "xmbc") {
          ...
          this.$refs["add"].validate((val) => {
            if (val) {
              ...
              this.tackdata.forEach((item) => param.accessoryList.push(item));
              // debugger;
              this.workflowRequest.setApiAfferentUrlOfPost(
                item.btnUrl,
                param,
    ...
    //       (res) => {
    //         //console.log(res.data,'留痕数据')
    //         this.trackData = res.data;
    //         this.trackLoading = false;
    //         var jsondata =
    //           res.data[1] && res.data[1].list
    //             ? JSON.parse(
    //                 res.data[1].list[2].fileList.substring(
    //                   1,
    //                   res.data[1].list[2].fileList.length - 1
    //                 )
    //               )
    //             : [];
    //         var jsondata1 =
    //           res.data[1] && res.data[1].list
    //             ? JSON.parse(
    //                 res.data[1].list[3].fileList.substring(
    //                   1,
    //                   res.data[1].list[2].fileList.length - 1
    //                 )
    //               )
    //             : [];
    //         this.tackdata.push(jsondata);
    //         this.tackdata.push(jsondata1);
    //         //console.log(this.tackdata,'最终数据')
    //       }
    //     );
    //   }
    // },
