...
import { api_queryProjectNotInIds } from "@/api/pilotMgr/index.js";
import {
  api_beCompletedList,
  api_queryIdList,
  api_getAmount,
  api_fileAway,
} from "@/api/beCompleted/index.js";
...
export default {
  ...
  data() {
    return {
      ...
      codeObject: {
        //上传文件权限code
        updateFileCode: "M006_004_005",
        //删除code
        deleteCode: "M006_004_006",
        //停止上传code
        stopUploadingCode: "M006_004_007",
      },
      //停止上传状态
      ...
      table: {
        ...
        tableConfig: {
          ...
          operateConfig: {
            ...
            export: this.export,
            exportPermissionCode: "M006_004_002",
          },
          emptyConfig: {
            // 表格数据为空时默认显示的内容
            display: true,
  ...
  methods: {
    ...
    btnValidate(item, row) {
      ...
      if (
        item.name == "上传附件" &&
        (this.$utils.checkCodeInArr("M006_004_004") ||
          this.$utils.checkCodeInArr("M006_004_005"))
      ) {
        return true;
      }
      return flag;
    ...
    edit(item) {
      removeSession("beCompletedAdd" + item.projectId);
      this.$router.push({
    ...
    export() {
      window.open(
        `${
          ...
          this.permissionCode
        }&access_token=${sessionStorage.getItem("token")}&type=2`
      );
    },
    //获取指定单位
    ...
    getTbData(index) {
      ...
      api_beCompletedList(params).then((res) => {
        if (res.msg.code == "0000") {
