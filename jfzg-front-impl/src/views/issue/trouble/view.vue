<template>
  <div class="total-page">
    ...
    <TestFormView
      :slotSrView="'questionLevel'"
      v-if="editableTabs.length"
      :editableTabs="editableTabs"
      :fromData="fromData"
    >
      <template slot="questionLevel" slot-scope="{ value }">
        <div>
          <img
    ...
    <sr-work-flow
      ...
      :backName="$route.query.backName ? $route.query.backName : 'trouble'"
    ></sr-work-flow>
  </div>
</template>
...
import { api_getQuestionDelitail } from "@/api/issue";
...
export default {
  ...
  methods: {
    ...
    async getListByTopicAndItem() {
      // 获取风险等级
      let res = await listByTopicAndItem({
        topic: "JFZG_DISPATCHQUESTION_LEVEL",
      });
      // 依据
      ...
      if (res?.data) {
        this.$set(this, "troubleList", res.data);
      }

      if (res2?.data) {
        this.$set(this, "troubleBasis", res2.data);
      }

      if (res3?.data) {
        this.$set(this, "troubleFrom", res3.data);
      }
      this.init();
    },
    async projectInit() {
    ...
    async init() {
      let res = await api_getQuestionDelitail({
        questionId: this.$route.query.id,
      });
      if (res?.data) {
        console.log(res.data);
        res.data.uploadFile = res.data.uploadFile
          ? JSON.parse(res.data.uploadFile)
          : [];
    ...
    changeSelectData(data) {
      ...
      for (const key in data) {
        ...
        } else if (key == "questionLevel") {
          data[key] = this.$utils.arrFind(
            this.troubleList,
            data[key],
            "itemCode"
          )?.chName;
        } else if (key == "questionFrom") {
          data[key] = this.$utils.arrFind(
            this.troubleFrom,
            data[key],
            "itemCode"
          )?.chName;
        } else if (key == "questionBasis") {
          data[key] = this.$utils.arrFind(
            this.troubleBasis,
            data[key],
            "itemCode"
