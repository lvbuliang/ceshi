<template>
  <div class="tabBtn">
    <el-radio-group
      ...
      fill="#F7F7F7"
    >
      <template v-if="userType === 'CJDW'">
        <el-radio-button label="CJDW"
          ><i class="base-iconfont icon-machinery"></i>承建单位</el-radio-button
        >
        <el-radio-button label="JSDW"
          ><i class="base-iconfont icon-company"></i>建设单位</el-radio-button
        >
        <el-radio-button label="JLDW"
          ><i class="base-iconfont icon-zhuanyequanwei"></i
          >监理单位</el-radio-button
        >
      </template>
      <template v-else-if="userType === 'JSDW'">
        <el-radio-button label="JSDW"
          ><i class="base-iconfont icon-company"></i>建设单位</el-radio-button
        >
        <el-radio-button label="CJDW"
      ...
      <template v-else-if="userType === 'JLDW'">
        <el-radio-button label="JLDW"
          ><i class="base-iconfont icon-zhuanyequanwei"></i
          >监理单位</el-radio-button
        >
    ...
    <div class="member_table_class">
      <div
        ...
        :style="{ marginRight: tabPosition === userType ? '315px' : '0' }"
      >
        <div class="left">
        ...
        <div class="center">
          ...
          <span class="content" style="height:80px"
            >项目负责人({{ fzrData }})</span
          >
          <span class="content bdNone" style="flex:1"
            >项目成员({{ MemberData }})</span
          >
        </div>
        <div class="right">
      ...
      <div v-if="tabPosition === userType" class="member_table_add">
        <selectUser :personnelList="Member_data" :personnelList2="Fzr_data" :buid="buid"> </selectUser>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getInfodata() {
      ...
      getDetailById({ id: this.$route.query.projectId }).then(res => {
        (this.constructionUnit = res.data.constructionUnit), // 建设单位id
          (this.contractUnit = res.data.contractUnit), // 承建单位id
          (this.controlUnit = res.data.controlUnit); // 监理单位id
        let obj = {
          contractUnit: res.data.contractUnit,
          constructionUnit: res.data.constructionUnit,
          controlUnit: res.data.controlUnit,
