...
import { api_pilotMgrList, api_getAllIds } from "@/api/pilotMgr/index.js";
...
export default {
  ...
  data() {
    return {
      ...
      codeObject: {
        //上传文件权限code
        updateFileCode: "M006_002_005",
        //删除code
        deleteCode: "M006_002_006",
        //停止上传code
        stopUploadingCode: "M006_002_007",
      },
      //停止上传状态
      ...
      table: {
        ...
        tableConfig: {
          ...
          operateConfig: {
            ...
            export: this.export,
            exportPermissionCode: "M006_002_002",
          },
          emptyConfig: {
            // 表格数据为空时默认显示的内容
            display: true,
  ...
  methods: {
    ...
    btnValidate(item, row) {
      ...
      if (
        item.name == "上传附件" &&
        (this.$utils.checkCodeInArr("M006_002_004") ||
          this.$utils.checkCodeInArr("M006_002_005"))
      ) {
        return true;
      }
      return flag;
    ...
    edit(item) {
      removeSession("pilotMgrAdd" + item.projectId);
      this.$router.push({
    ...
    export() {
      window.open(
        `${config.baseUrl}/jfzg/projectImpl/pilotRun/export?permissionCode=${
          this.permissionCode
    ...
    getTbData(index) {
      ...
      api_pilotMgrList(params).then((res) => {
        if (res.msg.code == "0000") {
