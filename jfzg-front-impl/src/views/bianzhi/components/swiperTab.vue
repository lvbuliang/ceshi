<template>
  <div class="Swiper">
    <Swiper ref="todoSwiper" :options="swiperOption" class="swiperTodo">
      <Swiper-slide :class="['swiperTodoItem', showButton  ? 'swiperTodoItemhid' : '']" v-for="(item, i) in eletab" :key="'s_' + i">
        <div @click="handleClick(i, $event)">
          ...
          <span :class="['cellClass', active == i ? 'activeClass' : '']" v-else :title='item.tagTypeName'>
            {{ item.tagTypeName.substring(0,8)+"..." }}
          </span>
...
.Swiper {
  ...
  .swiper-container {
    ...
    .swiperTodoItemhid:nth-last-child(1) {
      margin-right: 40px;
    }
  }
}
