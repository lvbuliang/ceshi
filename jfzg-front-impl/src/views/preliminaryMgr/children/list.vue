  ...
  <div class="preliminaryMgrList">
    ...
    >
      <!-- <template slot="cellSlot" slot-scope="scope">
        <div v-if="scope.scope.item.prop == 'status'">
          <span>{{ statusList[scope.scope.row.status] }}</span>
        </div>
      </template> -->
    </SrTable>
    <SrDialogSelect
...
import {
  ...
  api_queryProjectInitiativeTest,
} from "@/api/pilotMgr/index.js";
import {
  api_preliminaryMgrList,
  api_getAllIds,
  api_fileAway,
} from "@/api/preliminaryMgr/index.js";
...
export default {
  ...
  data() {
    return {
      ...
      codeObject: {
        //上传文件权限code
        updateFileCode: "M006_001_005",
        //删除code
        deleteCode: "M006_001_006",
        //停止上传code
        stopUploadingCode: "M006_001_007",
      },
      //停止上传状态
      ...
      getList: api_queryProjectInitiativeTest,
      userInfo: store.getters.userInfo,
      loading: false,
      permissionCode: "M006_001",
      statusList: {
        1: "待提交",
        2: "待检查",
        3: "待审查",
      ...
      table: {
        ...
        tableConfig: {
          ...
          operateConfig: {
            ...
            export: this.export,
            exportPermissionCode: "M006_001_002",
          },
          emptyConfig: {
            // 表格数据为空时默认显示的内容
            display: true,
  ...
  methods: {
    ...
    btnValidate(item, row) {
      ...
      if (
        ...
        (row.status == 1 || row.status == 5 || row.status == 7)
      ) {
        flag = true;
      }
      if (
        item.name == "上传附件" &&
        (this.$utils.checkCodeInArr("M006_001_004") ||
          this.$utils.checkCodeInArr("M006_001_005"))
      ) {
        return true;
      }
      return flag;
    ...
    edit(item) {
      removeSession("preliminaryMgrAdd" + item.projectId);
      this.$router.push({
    ...
    export() {
      window.open(
        `${config.baseUrl}/jfzg/projectImpl/firstCheck/export?permissionCode=${
          this.permissionCode
    ...
    getTbData(index) {
      ...
      api_preliminaryMgrList(params).then((res) => {
        if (res.msg.code == "0000") {
