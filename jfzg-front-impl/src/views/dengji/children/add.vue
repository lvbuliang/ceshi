    ...
    <div v-if="editableTabs.length > 0">
      <div v-if='!get_success'>
        ...
        <TestFormAdd
          ...
          :slotList="['基本信息']"
        >
          <ProjectBaseMessage slot="基本信息" :projectId="$route.query.projectId" />
        </TestFormAdd>
        <WorkFlow
          :hideFrom="true"
...
<script>
import {api_getlist_in_add,api_getlist_in_view} from "@/api/dengji/index";
...
export default {
  ...
  methods: {
    ...
    tg() {
      ...
      }else if(getSession("infoId_planedProject"+this.$route.query.projectId)&&getSession("infoId_planedProject"+this.$route.query.projectId) !="null"){
        data.id = getSession("infoId_planedProject"+this.$route.query.projectId)
        this.isNew = true;
      }else{
        this.isNew = false;
      }
    ...
    btg(str) {
      if(this.$refs["formAdd"]){
        ...
        api_getlist_in_add(data).then((res) => {
          if (res.msg.code == "0000") {
            if(str =='linshi_current'){
              setSession("infoId_planedProject"+this.$route.query.projectId, res.data);
            }else if(str =='close'){
              removeSession("infoId_planedProject"+this.$route.query.projectId);
    ...
    createdfunc(){
      ...
      if(this.$route.query.id){
        ...
        this.init(getSession("infoId_planedProject"+this.$route.query.projectId));
