<template>
  <div class="total-page">
    ...
    <sr-work-flow
      ...
      :backName="$route.query.backName?$route.query.backName:'everydaytrouble'"
    ></sr-work-flow>
  </div>
</template>
...
import { api_getSee } from "@/api/riskAndIssue";
...
export default {
  ...
  methods: {
    ...
    async init() {
      let res = await api_getSee({ id: this.$route.query.id });
      if (res?.data) {
        this.measureList = res.data?.measures?.map((item) => {
          if (!item.replyList) {
            if (item.replys) {
        ...
        res.data.problemType = obj[res.data.problemType]; */
